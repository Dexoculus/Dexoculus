[![ORCID](https://a11ybadges.com/badge?logo=orcid)][3][![PyPI](https://a11ybadges.com/badge?logo=pypi)][7] <br>

Hi, I'm **Hyeonbin Han**.<br>
I am an undergraduate student in the **Department of Mathematical Data Science** at Hanyang University ERICA.

Currently, I am working as a Junior Research Assistant in the Robot Intelligence Team (Humanoid TF) at CMES Robotics.
<br>

## **Education**

- **Hanyang University ERICA**, Ansan, Republic of Korea
  - B.Sc. in Mathematical Data Science (Mar. 2023 – Expected Feb. 2027)
  - GPA: 3.51 / 4.5

## **Research Interests**

- Physical AI & Robotics
- Imitation Learning
- Deep Reinforcement Learning
- Computer Vision
- Machine Learning & Deep Learning

## **Experience**

**CMES Robotics**, Seoul, Republic of Korea
*Junior Research Assistant, Robot Intelligence Team (Humanoid TF)* | Mar. 2026 – Jun. 2026
- Architected a scalable data acquisition and curation platform for the RB-Y1 humanoid robot, standardizing the teleoperation environment to ensure high-fidelity data collection.
- Deployed the Gello teleoperation framework for UR5 manipulators, streamlining the end-to-end pipeline from data curation to the training and physical deployment of Action Chunking Transformer (ACT) policies.

**Intelligence & Computation Lab, Hanyang University ERICA**
*Undergraduate Researcher* | Oct. 2023 – Jan. 2026
- Administered and optimized the laboratory's GPU training infrastructure, ensuring high availability and efficient resource allocation for deep learning workloads.
- Engineered robust data preprocessing pipelines for heterogeneous healthcare datasets (including mobile addiction metrics), transforming raw signals into machine-learning-ready formats.

## **Technical Skills**

- **Languages:** Python, C++ (Basic)
- **AI & Machine Learning:** PyTorch, Computer Vision (YOLOv8, EfficientNet), Imitation Learning (ACT, Diffusion Policy), Federated Learning (Flower)
- **Robotics & Edge:** Manipulator Integration (UR5, RB5-850), Edge Inference (Raspberry Pi), Teleoperation (Gello), 3D Perception
- **DevOps & Infrastructure:** Docker, Git, GitHub, GitLab, Linux, GPU Server Administration
- **Deployment & Tools:** Network & DNS Management (Cloudflare Tunnels), Streamlit

## **Selected Projects**

[![HYU-Internship2-2025H2/HYU-Internship2-2025H2 - GitHub](https://gh-card.dev/repos/HYU-Internship2-2025H2/HYU-Internship2-2025H2.svg)](https://github.com/HYU-Internship2-2025H2/HYU-Internship2-2025H2)
*Sep. 2025 – Dec. 2025*
<br>

[![Dexoculus/canon-control-pannel-cls - GitHub](https://gh-card.dev/repos/Dexoculus/canon-control-pannel-cls.svg)](https://github.com/Dexoculus/canon-control-pannel-cls)
*Sep. 2025 – Dec. 2025*
<br>

[![Dexoculus/box_depalletizing - GitHub](https://gh-card.dev/repos/Dexoculus/box_depalletizing.svg)](https://github.com/Dexoculus/box_depalletizing)
*Mar. 2025 – Jun. 2025*
<br>

[![Dexoculus/hand-held-camera-display-state-classifiation - GitHub](https://gh-card.dev/repos/Dexoculus/hand-held-camera-display-state-classifiation.svg)](https://github.com/Dexoculus/hand-held-camera-display-state-classifiation)
*Mar. 2025 – Jun. 2025*

## **Honors & Awards**

- **Excellence Award**, 2025 Intelligent Robot WE-Meet Project Integrated Competition (Jan. 2026)
- **Participation Award**, 2025 Shipbuilding & Maritime Big Data Utilization Idea Contest (Dec. 2025)
- **Best Paper Award**, 2025 KIICE Fall Conference (Oct. 2025)
- **Bronze Prize** (Robot News President's Award), KIICE Physical AI Challenge (Oct. 2025)
- **Excellence Award**, 2024 Hanyang University SW/ICT/AI Integrated Conference (Dec. 2024)

## **Publications**

- **H. Han**, K. Y. Lee, S. Y. Shin, Y. Kim, G. Jo, J. Park, & Y. M. Kim, "*Prediction of Closed Quotient During Vocal Phonation using GRU-type Neural Network with Audio Signals*," J. Inf. Commun. Converg. Eng. (JICCE), vol. 22, no. 2 (2024).

## **Conference Presentations**

- 2025 Korean Institute of Information and Communication Engineering (KIICE) Fall Conference,<br>
  "*DMD Method for Analyzing Interactions Between Board Game Players*" (poster)
- 2024 Korean Institute of Intelligent Systems (KIIS) Spring Conference,<br>
  "*Deep learning-based algorithm for analyzing EGG signals to predict closed quotient rate*" (poster)
- 2024 Korean Institute of Information and Communication Engineering (KIICE) Spring Conference,<br>
  "*GRU-based algorithm for predicting closed quotient rate*" (poster)

<br>

[![Github Pages](https://img.shields.io/badge/github%20pages-121013?style=for-the-badge&logo=github&logoColor=white)][5]<br>

[1]: https://sites.google.com/view/gwanghyunjo/home
[2]: https://scholar.google.com/citations?hl=ko&user=8BN1dJ7aXgMC&view_op=list_works&sortby=pubdate
[3]: https://orcid.org/0009-0007-4713-5892
[4]: https://www.linkedin.com/in/hyeonbin-han-869312279/
[5]: https://page.dexoculus.com/
[6]: https://www.kaggle.com/dexoculus
[7]: https://pypi.org/user/dexoculus/
